...
function localTutorAnswer(doc, question){
  ...
  if(!source) return "There is not enough study material loaded yet.";
  const sentences=extractSentences(source,80);
  const terms=(doc?.terms||[]).map(t=>typeof t==='string'?t:t?.term||t?.label||"").filter(Boolean);
  const qWords=new Set(q.split(/[^a-z0-9]+/).filter(w=>w.length>=3 && !STOP.has(w)));
  const scored=sentences.map((text,i)=>{
    const words=text.toLowerCase().split(/[^a-z0-9]+/).filter(Boolean);
    let score=0;
    for(const w of words) if(qWords.has(w)) score+=2;
    for(const t of terms){const tl=String(t).toLowerCase();if(tl && text.toLowerCase().includes(tl)) score+=3;}
    if(/what is|define|meaning|definition/.test(q)&&/\bis\b|means|refers to|defined as/i.test(text)) score+=2;
    return {text,i,score};
  }).sort((a,b)=>b.score-a.score).filter(x=>x.score>0).slice(0,4);
  if(!scored.length) return "I could not find enough evidence for that question in the loaded material. Try using the exact term or phrase from the lesson.";
  const evidence=scored.map(x=>`• ${x.text}`).join("\n");
  return `Based on your study material:\n\n${evidence}\n\nStudy tip: compare these points with the original page before memorizing them.`;
}
...
function renderAI(){
  ...
  if(info)info.textContent=ai.model?`${ai.model} • ${ai.device||"local"}`:`Model: on-device Qwen3 0.6B • Android uses an adaptive GPU/CPU path • first load downloads model files`;
  const settingsInfo=$("#settingsAiInfo");if(settingsInfo)settingsInfo.textContent=state.settings.ai?.enabled?"AI enabled. Android uses a Web Worker and adaptive GPU/CPU path so the UI remains interactive.":"AI is optional; deterministic review works without it.";
}
...
function bind(){
  ...
  $("#aiAsk").onclick=async()=>{const d=activeDoc(),q=$("#aiQuestion").value.trim(),out=$("#aiAnswer");if(!d)return toast("Select a study material first.","error");if(!q)return toast("Write a question first.","error");out.textContent="Searching your material…";try{const instant=localTutorAnswer(d,q);out.textContent=instant;toast("Instant source-grounded answer ready.","success");if(aiWorker&&state.settings.ai?.enabled){try{const r=await aiRequest("ask",{source:aiQuestionSource(d,q),profile:learnerDigestForAI(),question:q});if(r?.text)out.textContent=r.text;}catch(e){console.warn("Optional neural tutor failed",e);}}}catch(e){out.textContent=e.message||"Could not answer from this material.";toast("No matching evidence was found.","error");}};
